...
export const ChapterList = ({onEdit, onReorder, items}: ChapterListProps) => {
  ...
  const onDragEnd = (result: DropResult) => {
    ...
    const balkUpdateData = updatedChapters.map((chapter) => ({
      id: chapter.id,
      position: items.findIndex((item) => item.id === chapter.id),
    }))

    onReorder(balkUpdateData)
  }
            ...
            {chapters.map((chapter, index) => (
              ...
              >
                {(provided) => (
                  <div
                    className={cn(
                      `flex items-center gap-x-2 bg-slate-200 border-slate-200 border text-slate-700 
